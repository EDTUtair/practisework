...
Процедура ОбработчикОжиданияДлительнойОперации()
	...
	//<ШАБЛОН: ЗДЕСЬ ОПИСЫВАЕТСЯ ОБРАБОТЧИК ОЖИДАНИЯ ДЛИТЕЛЬНОЙ ОПЕРАЦИИ>
	...
	Для каждого СтрокаРодителя2 Из дзСписокСуммКПеречислению.Строки Цикл
		...
		Для Каждого СтрокаРодителя Из СтрокаРодителя2.Строки Цикл
			...
			Для каждого СтрокаТаблицы Из СтрокаРодителя.Строки Цикл
				...
				//СтрокаДокумента.ДатаПолученияДохода = СтрокаТаблицы.ДатаПолученияДохода;
				//СтрокаДокумента.РеквизитыПлатежногоПоручения = 
...
Процедура УстановитьФлажкиВСписке(пФлаг)
	...
	КонецЦикла;	
	//
	//Для каждого СтрокаДерева Из СписокСуммКПеречислению Цикл
	//
	//	СтрокаДерева.Пометка = пФлаг;
	//
	//КонецЦикла;
